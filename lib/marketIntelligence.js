...
export async function analyzeMarketIntelligence({ leadId }) {
  ...
  try {
    ...
    const system =
      ...
      "social URLs ONLY if they appear in the provided content. " +
      "PAID-ADS EVIDENCE BAR (strict): treat 'paid ads' as CURRENTLY-RUNNING paid " +
      "advertising — e.g. an ad library showing active ads, or a source explicitly " +
      "stating the business runs paid Meta/Facebook/Instagram/Google ads. The mere " +
      "existence of a Facebook Page, Instagram profile, or any other social account " +
      "is NOT evidence of paid ads. CRITICAL: never claim exact ad spend or an exact " +
      "active-ad count unless a provided source explicitly states a number. " +
      "Respond with ONLY a valid JSON object (no markdown/code fences) with EXACTLY " +
      "these keys: " +
      '"paid_ads_evidence" (boolean — true ONLY if a source gives explicit evidence of ' +
      "currently-active PAID ads; social-profile presence alone = false), " +
      '"meta_ads_active" (boolean — true ONLY if paid_ads_evidence is true AND the paid ' +
      "ads are on Meta/Facebook/Instagram; otherwise false), " +
      '"active_ad_count_estimate" (number|null — ONLY if a source states a count; else null), ' +
      '"ad_platforms_seen" (array of PAID ad platforms with explicit evidence ONLY, e.g. ' +
      '["Meta/Facebook","Google"]; do NOT list a platform just because a social profile ' +
      'exists; [] when there is no paid-ad evidence), ' +
      ...
      '"explanation" (string: cite which signals drove the assessment, and state ' +
      "EXPLICITLY whether public paid-ad evidence was found or not), " +
    ...
    // ---- Paid-ads guard (deterministic; social presence ≠ paid ads) -------
    // Only treat paid ads as real when the model reports EXPLICIT paid-ad
    // evidence. Otherwise force meta_ads_active=false, no paid platforms, and
    // no ad-count estimate — social presence stays in the social fields only.
    const paidAds = parsed.paid_ads_evidence === true;
    let meta_ads_active;
    if (paidAds) {
      meta_ads_active = parsed.meta_ads_active === false ? false : true;
    } else if (parsed.paid_ads_evidence === false) {
      meta_ads_active = false; // evidence reviewed, none found
    } else {
      meta_ads_active = null; // indeterminate
    }
    const active_ad_count_estimate =
      paidAds && typeof parsed.active_ad_count_estimate === "number"
        ? parsed.active_ad_count_estimate
        : null; // null unless a source states a count AND paid ads are evidenced

    // PAID ad platforms only; empty unless there is explicit paid-ad evidence.
    const platforms =
      paidAds && Array.isArray(parsed.ad_platforms_seen)
    ...
    const market_intelligence = {
      status: "analyzed",
      created_at,
      meta_ads_active, // true ONLY with explicit active paid-ad evidence (guard above)
      active_ad_count_estimate, // null unless a source states a count AND paid ads evidenced
      ad_platforms_seen: platforms, // PAID ad platforms only; [] when no paid-ad evidence
      ad_themes: themes,
