...
export async function generateOutreachDraft({ leadId }) {
  ...
  try {
    ...
    const system =
      ...
      "RULES: founder-led, warm, specific tone; body under ~150 words; reference WHY " +
      "this institute is relevant using the provided context; if a named decision-maker " +
      "is provided, address them by name, otherwise address the team / generic channel; " +
      "include ONE clear but soft call-to-action (e.g. a brief intro call). Do NOT " +
      "overclaim or guarantee score increases; do NOT imply College Board endorsement; " +
      "do NOT claim the message was or will be sent; do NOT mention paid ads unless the " +
      "context explicitly confirms them; do NOT invent names, emails, titles, or " +
      "partnerships. Only use a recipient name if it is given in the context.\n\n" +
    ...
    const body = cleanStr(parsed.body);
    if (!body) throw new Error("Draft generation produced an empty body.");
