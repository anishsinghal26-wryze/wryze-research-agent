// ============================================================================
// app/sales-pipeline/api/approvals/reject/route.js
// ----------------------------------------------------------------------------
// POST /api/approvals/reject
//
// Human rejection action. Marks an approval_queue item rejected and updates the
// linked entity's status (e.g. outreach_drafts -> rejected). Emits
// approval_rejected. NEVER sends anything.
//
// Body: { "approval_id": "uuid", "decision_notes": "optional" }
//
// Gate: requires the sales-pipeline session cookie (sp_auth).
// ============================================================================
...
export async function POST(request) {
  ...
  if (row.entity_type === "outreach_draft") {
    const draftUpdated = await setOutreachDraftStatus(row.entity_id, "rejected");
    if (!draftUpdated) {
      return Response.json(
  ...
  await emitEvent("approval_rejected", {
    task_id: row.task_id,
    lead_id: row.lead_id,
  ...
  return Response.json({ ok: true, approval_id, status: "rejected" });
}
